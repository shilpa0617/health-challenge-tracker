...
describe('AppComponent', () => {
  ...
  it('should load and group workout entries on init', () => {
    expect(component.workoutEntries.length).toBe(6);
    expect(component.groupedWorkoutEntries.length).toBe(3); // Grouped by user names
  });
  ...
  it('should add a new workout entry', () => {
    ...
    expect(component.workoutEntries.length).toBe(7);
    expect(component.groupedWorkoutEntries.length).toBe(4); // A new user 'Test User' added
    expect(localStorage.setItem).toHaveBeenCalled();
    expect(component.updateChart).toHaveBeenCalled();
    expect(mockForm.resetForm).toHaveBeenCalled();
  });

  it('should filter and paginate entries', () => {
    component.searchTerm = 'John';
    component.applyFilters();
    expect(component.paginatedEntries.length).toBe(2); // Both John Doe entries

    component.itemsPerPage = 1;
    component.applyFilters();
    expect(component.paginatedEntries.length).toBe(1); // 1 per page
    expect(component.totalPages).toBe(2); // 2 pages
  });
  ...
  it('should get unique users', () => {
    ...
    expect(users.length).toBe(3); // John Doe, Jane Smith, Mike Johnson
  });
});
