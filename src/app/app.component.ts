        ...
        <table>
          ...
          <tbody>
            <tr *ngFor="let entry of paginatedEntries">
              ...
              <td>{{entry.totalWorkoutMinutes}}</td>
            </tr>
          </tbody>
        </table>
...
export class AppComponent implements OnInit {
  ...
  groupedWorkoutEntries: GroupedWorkoutEntry[] = [];
  paginatedEntries: GroupedWorkoutEntry[] = [];
  ...
  groupWorkoutEntries() {
    const groupedData: { [userName: string]: GroupedWorkoutEntry } = {};
    ...
    this.workoutEntries.forEach(entry => {
      ...
      if (!groupedData[entry.userName].workouts.includes(entry.workoutType)) {
        groupedData[entry.userName].workouts.push(entry.workoutType);
      }
      groupedData[entry.userName].totalWorkoutMinutes += entry.workoutMinutes;
    });
  
    this.groupedWorkoutEntries = Object.values(groupedData);
    this.applyFilters();
  }
  ...
  applyFilters() {
    let filteredEntries = this.groupedWorkoutEntries;
    ...
    if (this.filterType) {
      filteredEntries = filteredEntries.filter(entry =>
        entry.workouts.some(workout => workout.includes(this.filterType))
      );
    }

    this.totalPages = Math.ceil(filteredEntries.length / this.itemsPerPage);
    this.currentPage = Math.min(this.currentPage, this.totalPages);
    this.paginatedEntries = filteredEntries.slice((this.currentPage - 1) * this.itemsPerPage, this.currentPage * this.itemsPerPage);
  }
